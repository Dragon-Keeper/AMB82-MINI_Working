 ...
 * 最后修改: 2026-05-05
 * 作者/维护: Dragon-Keeper
 * 联系邮箱: Dragon-Keeper@Github.com
 *
 * =============================================================================
 * 项目概述
 * =============================================================================
 * 本项目是基于 Realtek AmebaPro2 (AMB82-MINI) 开发板的相机控制系统，集成了
 * 实时预览、拍照、720P视频录制（带音频）、图片/视频回放（带音频）、ISP参数调节、
 * WiFi连接、OTA在线升级、USB大容量存储及BLE WiFi配网等丰富功能。系统采用
 * FreeRTOS多任务架构，通过EC11旋转编码器控制菜单操作，采用ST7789 240x320 TFT
 * 屏幕显示中文界面。
 *
 * =============================================================================
 * 硬件配置
 ...
 * - 音频采集: INMP441 I2S麦克风 (16kHz/16bit/单声道, I2S1_RX)
 * - 音频播放: MAX98357A I2S功放模块 (16kHz/16bit/立体声输出, I2S1_TX)
 * - 电源要求: 5V直流输入
 * - 注意: 录音与播放分时复用I2S1接口(D14/D15/D17)，不可同时进行
 *
 * =============================================================================
 * 软件架构
 * =============================================================================
 ...
 * =============================================================================
 * [A] 实时预览 & 拍照
 *     - VGA(640x480)实时预览, 720P(1280x720)拍照
 *     - 支持亮度/对比度/饱和度/曝光/白平衡调节
 *     - 时间戳命名 (IMG_YYYYMMDD_HHMMSSXXX.jpg)
 *
 * [B] 视频录制
 *     - 720P MJPEG编码 + 16kHz PCM音频同步录制
 *     - AVI格式封装 (独立视频/音频轨道)
 *     - 录制帧率: 15fps, 双通道(录制+预览)协作
 *     - 自动保存到SD卡
 *
 * [C] 媒体回放
 *     - JPG图片查看 (720P自动缩放至240x320屏幕)
 *     - AVI视频播放 (MJPEG解码 + PCM音频解码)
 *     - MAX98357A音频输出 (16kHz立体声, I2S1_TX)
 *     - 音频预加载机制: 预填64个音频块确保播放流畅
 *     - 帧缓冲区回放模式: 320x180帧缓冲+一次性DMA传输
 *     - 四格缩略图布局, LRU缓存(500缩略图/约25MB)
 *     - 旋转编码器切换文件, 按压播放/返回
 *
 * [D] ISP参数设置
 *     - 实时亮度/对比度/饱和度/曝光/白平衡调节
 *     - 色条可视化UI, 参数持久化存储(Flash)
 *     - 左侧预览+右侧参数面板双栏布局
 *
 * [E] USB大容量存储 / WiFi文件传输
 *     - USB MSC模式: 开发板作为U盘连接电脑传输文件
 *     - WiFi文件服务器: HTTP服务器下载SD卡文件
 *
 * [F] 系统设置
 *     - 子菜单: 时间设置/重启相机/BLE WiFi配网/OTA升级/版本信息/返回
 ...
 * | TASK_FUNCTION_E     | USB/WiFi文件传输| 1      | 8192    |
 ...
 * - 共享模块: 全局定义/类型/资源配置
 * - 工具模块: 日志/定时器/缓冲区管理
 * - 显示模块: TFT管理/DMA SPI/字库渲染/图形基元
 * - 编码器模块: EC11旋转编码器驱动(中断+消抖)
 * - 相机模块: 相机管理/SD卡管理/ISP配置
 * - 音视频模块: MJPEG编解码/INMP441录音/MAX98357A播放/AVSync同步
 * - USB模块: USB大容量存储设备模拟
 * - 菜单模块: 菜单管理/参数设置/三角形光标控制/上下文管理
 * - 任务管理: 任务工厂/任务管理器/事件组通信
 * - 系统模块: 状态管理/配置管理/WiFi连接/OTA升级
 *
 * =============================================================================
 * 关键技术要点
 * =============================================================================
 ...
 * - 双通道相机: 独立配置预览通道(VGA)和录制/拍照通道(720P)
 * - 事件驱动: FreeRTOS事件组实现任务间通信和同步
 * - WiFi持久化: FlashMemory存储WiFi凭据(0xFE000, 4KB扇区, 最多5组)
 * - BLE配网: BLE服务UUID: FF01, 广播名: Ameba_XXXXXX, 超时120秒
 * - I2S分时复用: INMP441(RX)与MAX98357A(TX)共享I2S1, 录制/播放互斥
 * - 音频预加载: 视频播放前预填64个音频块缓冲, 防止初始掉帧
 * - LRU缩略图缓存: 500个缩略图上限, LRU淘汰策略管理内存
 *
 * =============================================================================
 * 使用注意事项
 * =============================================================================
 ...
 * 5. 视频录制时请保持INMP441麦克风连接以获得同步音频
 * 6. 视频回放时请连接MAX98357A功放和扬声器以获得音频输出
 * 7. OTA升级前请确保WiFi连接稳定
 *
 * =============================================================================
 * WiFi默认配置
 ...
 * | SSID      | 密码       |
 * |-----------|------------|
 * | Force     | XXXXXXXX   | 在225行处修改为正确WIFI密码
 * | Tiger     | XXXXXXXX  |
 *
 * =============================================================================
 * NTP服务器列表 (按优先级排序)
