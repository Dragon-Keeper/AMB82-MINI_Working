...
public:
    ...
    // 写入文件，可选参数指定文件最后修改时间
    int32_t writeFile(const char* path, const uint8_t* data, uint32_t size, const DS3231_Time* time = nullptr);
    ...
    bool setLastModTime(const char* path, const DS3231_Time& time);
