...
uint8_t TwoWire::requestFrom(uint8_t address, uint8_t quantity, uint8_t sendStop)
{
    ...
    int read = i2c_read(((i2c_t *)this->pI2C), ((int)address), reinterpret_cast<char *>(this->rxBuffer), ((int)quantity), ((int)sendStop));

    // Check if read was successful
    if (read < 0) {
        // I2C read failed, no data available
        rxBufferIndex = 0;
        rxBufferLength = 0;
        return 0;
    }
...
uint8_t TwoWire::endTransmission(uint8_t sendStop)
{
    ...
    // Reset flag before transmission
    flag = 0;
    ...
    if (length < 0) {
        ...
        // No ACK received - device not responding
        error = 2;
    } else {
        // Success
        error = 0;
