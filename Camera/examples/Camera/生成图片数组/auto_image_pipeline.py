...
def generate_config(h_files):
    ...
    config_content = """// 图像配置文件 - 统一管理所有图像变量名
// 自动生成于"生成图片数组"目录
// 请将此文件复制到项目目录替换旧文件
...
"""
    ...
    # 写入文件
    with open("ImageConfig.h", "w", encoding="utf-8") as f:
        f.write(config_content)

    print(f"✅ 成功生成 ImageConfig.h")
    print("\n生成的宏定义:")
    for name in image_names:
        prefix = name.upper()
...
def main():
    ...
    for h_file in h_files:
        print(f"  • {h_file}")
    print(f"  • ImageConfig.h")

    print(f"\n📋 使用说明:")
    print("1. 将此 ImageConfig.h 文件复制到项目目录")
    print("2. 替换原有的 ImageConfig.h 文件")
